...
def create_pie_chart_results(stats):
    ...
    labels = ["Emp ID Updated", "Emp ID Updated & Project Mismatch", "Name MisMatch"]
    values = [
...
def create_bar_chart_emp_ids(stats):
    ...
    values = [
        stats["blank_emp_ids_count"],
        stats["updated_emp_ids"],
        stats["blank_emp_ids_count"] - stats["updated_emp_ids"],
    ]
...
def create_donut_chart_success(stats):
    ...
    total = stats["total_records_in_sow"]
...
def display_status_wise_breakdown(stats):
    """Display Status-wise Breakdown of Name and Project Mismatches"""

    st.markdown("#### Status-wise Comparison Breakdown")
    ...
    for status_label, status_key in status_groups:
        total = stats.get(f"{status_key}_total", 0)
        if total > 0:
            name_mismatch = stats.get(f"{status_key}_name_mismatch", 0)
            project_mismatch = stats.get(f"{status_key}_project_mismatch", 0)
            perfect = stats.get(f"{status_key}_perfect", 0)
    ...
    if breakdown_data:
        ...
        st.info("No status-wise breakdown data available")


def display_metrics(stats):
    """Display SOW Summary Metrics as cards"""
    ...
    with col1:
        st.markdown(
            ...
            <div class="metric-card">
                <div class="metric-label">Total SOW Records</div>
                <div class="metric-value">{format_number(stats["total_records_in_sow"])}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with col2:
        st.markdown(
            ...
            <div class="metric-card updated">
                <div class="metric-label">Blank Emp IDs</div>
                <div class="metric-value">{format_number(stats["blank_emp_ids_count"])}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )

    # Row 2: Overall Results
    ...
    with col3:
        st.markdown(
            ...
            <div class="metric-card perfect-match">
                <div class="metric-label">Emp ID Updated</div>
    ...
    with col4:
        st.markdown(
            ...
            <div class="metric-card mismatch-project">
                <div class="metric-label">Emp ID Updated & Project Mismatch</div>
    ...
    with col5:
        st.markdown(
            ...
            <div class="metric-card mismatch-name">
                <div class="metric-label">Name MisMatch</div>
    ...
    with col6:
        st.markdown(
            f"""
            <div class="metric-card updated">
                <div class="metric-label">Total IDs Updated</div>
...
if run_comparison:
    if base_file is not None and sow_file is not None:
        # Create containers for loading animation
        progress_container = st.container()

        with progress_container:
            st.info("⏳ Starting comparison process...")

            # Show loading steps
            with st.spinner("📥 Loading Base file..."):
                time.sleep(0.3)

            with st.spinner("📥 Loading SOW file..."):
                time.sleep(0.3)

            with st.spinner("🔍 Validating columns..."):
                time.sleep(0.3)

            with st.spinner("⚙️ Building lookup database..."):
                time.sleep(0.3)
...
if st.session_state.comparison_done and st.session_state.stats is not None:
    ...
    with col2:
        ...
        stats_text = f"""
        - **Total SOW Records:** {format_number(stats["total_records_in_sow"])}
        - **Blank Employee IDs:** {format_number(stats["blank_emp_ids_count"])}
        - **Emp ID Updated:** {format_number(stats["perfect_matches"])}
        - **Emp ID Updated & Project Mismatch:** {format_number(stats["project_mismatches"])}
        - **Name MisMatch:** {format_number(stats["name_mismatches"])}
        - **Total IDs Updated:** {format_number(stats["updated_emp_ids"])}
        """
    ...
    display_status_wise_breakdown(stats)
    ...
    if total_pages > 0:
        ...
        if "Employee Name" in display_df.columns:
            columns_to_show.append("Employee Name")
        if "Mphasis Emp Id" in display_df.columns:
            columns_to_show.append("Mphasis Emp Id")
        if "Project Id" in display_df.columns:
            columns_to_show.append("Project Id")
        if "Result" in display_df.columns:
            columns_to_show.append("Result")
        if "PRISM Project Id" in display_df.columns:
            columns_to_show.append("PRISM Project Id")

        # If columns_to_show is empty, show all columns
        if columns_to_show:
...
SUMMARY STATISTICS
Total Records: {format_number(stats["total_records_in_sow"])}
...
Blank IDs Found: {format_number(stats["blank_emp_ids_count"])}
Updated IDs: {format_number(stats["updated_emp_ids"])}

PERCENTAGES
Perfect Match Rate: {stats["perfect_matches"] / max(stats["total_records_in_sow"], 1) * 100:.2f}%
Project Mismatch Rate: {stats["project_mismatches"] / max(stats["total_records_in_sow"], 1) * 100:.2f}%
Name Mismatch Rate: {stats["name_mismatches"] / max(stats["total_records_in_sow"], 1) * 100:.2f}%
Update Success Rate: {stats["updated_emp_ids"] / max(stats["blank_emp_ids_count"], 1) * 100:.2f}%
"""
